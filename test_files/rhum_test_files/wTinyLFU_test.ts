...
Rhum.testPlan("WTinyLFU cache functionality", () => {
  ...
  Rhum.testSuite("Window cache functionality", () => {
    Rhum.testCase(
      "should add new item to the windowLRU when adding to WTLFU cache",
      () => {
    ...
    Rhum.testCase(
      "should move items ejected from windowLRU into the probationaryLRU cache",
      async () => {
        const cache = new WTinyLFUCache(100);
    ...
    Rhum.testCase(
      "should promote items from probationaryLRU to the protectedLRU when accessed",
      async () => {
        const cache = new WTinyLFUCache(100);
        await cache.putAndPromote("one", 1);
        await cache.putAndPromote("two", 2);
    ...
    Rhum.testCase(
      "should demote items ejected from protectedLRU to probationary LRU",
      async () => {
        const cache = new WTinyLFUCache(100);
    ...
    Rhum.testCase(
      "should move highest frequency item into full probationary cache",
      async () => {
        const cache = new WTinyLFUCache(100);
    ...
    Rhum.testCase(
      "should evict least recently used item from WLRU",
      async () => {
    ...
    Rhum.testCase(
      "should evict least recently used item from ProbationaryLRU",
      async () => {
        const cache = new WTinyLFUCache(100);
    ...
    Rhum.testCase(
      "should evict least recently used item from ProtectedLRU",
      async () => {
        const cache = new WTinyLFUCache(100);
